...
def search():
    ...
    if year:
        cur.execute('''
            ...
            FROM Movie 
            WHERE release_year = ? AND title LIKE ? AND user_id = ?
            GROUP BY id
        ''', (year, f'%{query}%', user_id))
        ...
        if search_type == 'person' or search_type == 'all':
            cur.execute('''
                ...
                WHERE p.name LIKE ? AND p.user_id = ?
                GROUP BY p.id
            ''', (f'%{query}%', user_id))
...
def movie_list():
    ...
    return render_template('movie_list.html', username=username)
...
def update_all_average_ratings():
    ...
    cur.execute('''
        ...
        SELECT imdbID, ROUND(AVG(user_rating), 1) AS average_rating
        FROM Movie
