...
def person_detail(person_id):
    ...
    cur.execute('''
        SELECT name, birthday, image_url, bio 
        FROM Person 
        WHERE id = ? AND user_id = ?
    ''', (person_id, user_id))
    person = cur.fetchone()
    ...
    return render_template('person_detail.html', person_id=person_id, name=name, birthday=birthday, image_url=image_url, bio=bio, movies=movies, person=person, comments=comments, person_name=name)
    """return render_template(
        'person_detail.html',
        person_id=person_id,
        person=person,
        comments=comments,
        person_name=name  # Ensure this variable is passed
    )"""
